...
@interface RKMarqueeView () <RKMarqueeViewTouchResponder>

@property (nonatomic, strong) UIView *contentView;
@property (nonatomic, assign) NSInteger visibleItemCount;
@property (nonatomic, strong) NSMutableArray<RKMarqueeItemView*> *items;
@property (nonatomic, assign) int firstItemIndex;
@property (nonatomic, assign) int dataIndex;
@property (nonatomic, strong) NSTimer *scrollTimer;
@property (nonatomic, strong) RKMarqueeViewTouchReceiver *touchReceiver;
...
- (void)resetAll {
    ...
    for (int i = 0; i < _visibleItemCount + 2; i++) {
        RKMarqueeItemView *itemView = [[RKMarqueeItemView alloc] init];
...
- (void)createItemView:(RKMarqueeItemView*)itemView {
...
- (void)updateItemView:(RKMarqueeItemView*)itemView atIndex:(NSInteger)index {
    if (index < 0) {
        [itemView clear];
    }
...
- (void)resetTouchReceiver {
    if (_touchEnabled) {
        if (!_touchReceiver) {
            self.touchReceiver = [[RKMarqueeViewTouchReceiver alloc] init];
...
- (void)touchesEndedAtPoint:(CGPoint)point {
    for (RKMarqueeItemView *itemView in _items) {
...
@implementation RKMarqueeViewTouchReceiver
...
@implementation RKMarqueeItemView
